...
class RoboneoBotTester(Node):
    ...
    def auto_movement(self):
        ...
        if self.distance < 10.0 and self.robot_state == 'IDLE':
            self.get_logger().info('Distance under 10 CM - Stopping motor')
            # Stop the motor
            ...
            if self.left_distance > 0.0 and self.right_distance > 0.0:
                if self.left_distance > self.right_distance:
                    ...
                    self.send_twist_command(0.0, 0.7)  # Turn left
                    ...
                    self.send_twist_command(0.0, -0.7)  # Turn right
                    self.robot_state = 'TURNING'
                    # Timer to stop turning after 1 second
    ...
    def measure_left_distance(self):
        ...
        self.send_twist_command(0.7, 0.7)  # Turn left slowly
    ...
    def measure_right_distance(self):
        ...
        self.send_twist_command(0.7, -0.7)  # Turn right slowly
        self.robot_state = 'MEASURING_RIGHT'
        
        # Timer to stop turning and measure distance
        self.delay_timer = self.create_timer(1.0, self.stop_and_measure_right)
